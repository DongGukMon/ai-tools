...
/** Listen for backend log events and pipe to console. Call once at app init. */
export async function initBackendLogPipe() {
  if (!DEBUG) return;
  const { listen } = await import("@tauri-apps/api/event");
  listen<{ level: string; tag: string; message: string }>("grove:log", (event) => {
